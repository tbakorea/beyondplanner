    ...
    def handle_get_state(self):
        ...
        if planner_storage() == "supabase-db":
            if not token:
                ...
                return
            try:
                self.write_json(200, get_supabase_planner_state(token, request_since(self.path, self.headers)))
                return
...
def request_since(path: str, headers) -> str:
...
def get_supabase_planner_state(token: str, since: str = "") -> dict:
    supabase_url, anon_key = supabase_base()
    ...
    if since:
        ...
        if server_updated_at and not is_newer_timestamp(server_updated_at, since):
            return {
                "exists": True,
                "state": None,
