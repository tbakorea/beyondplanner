...
const settingsLanguageLabels = {
  en: {
    ...
    appGroups: [
      ...
      ["Operations & Data", "Projects, AI, user approval, and backup in one place."],
    ],
    profileSummary: {
      eyebrow: "Profile Setup",
      title: "Start with five answers.",
      items: [
        ["Basic", "Identity, work, goal, issue, coaching style"],
        ["Optional", "Mission, values, roles, rhythm, health"],
        ["Result", "Better decisions without a long form"],
      ],
    },
    ...
    steps: [
      ["Basic Profile", "Only the fields AI coaching uses immediately."],
    ],
    benefit: "Five short answers are enough to start. Open optional details only when you want more precise coaching.",
    profileMap: ["Basics", "Optional"],
    profileStages: [
      ["Basics", "Ready-to-use profile", "Short answers are enough. One clear line per field is better than a long essay."],
      ["Optional", "Open only when useful", "Mission, roles, rhythm, and health stay available without crowding the first screen."],
    ],
    profileLabels: {
  ...
  ko: {
    ...
    appGroups: [
      ...
      ["운영과 데이터", "프로젝트, AI, 사용자 승인, 백업을 한곳에서 관리합니다."],
    ],
    profileSummary: {
      eyebrow: "사용자 입력",
      title: "처음에는 5개만 입력합니다",
      items: [
        ["기본", "정체성, 하는 일, 목표, 고민, 코칭 방식"],
        ["선택", "사명, 가치, 역할, 리듬, 건강"],
        ["효과", "긴 설문 없이 오늘 판단부터 좋아집니다"],
      ],
    },
    primaryLanguage: "언어",
    appItems: [
    ...
    steps: [
      ["기본 입력", "AI 코칭에 바로 필요한 정보만 먼저 입력합니다."],
    ],
    benefit: "아래 5개만 있어도 오늘의 우선순위, 일정 배치, 코칭 문장이 사용자 상황에 맞게 조정됩니다.",
    profileMap: ["기본 입력", "선택 입력"],
    profileStages: [
      ["기본 입력", "오늘부터 바로 쓰는 정보", "길게 쓰지 않아도 됩니다. 한 줄씩만 적어도 충분합니다."],
      ["선택 입력", "더 정확한 코칭을 원할 때만 열기", "사명, 역할, 실행 리듬, 건강 정보는 필요할 때 천천히 채웁니다."],
    ],
    profileLabels: {
...
function restoreUndoAction() {
  ...
  if (snapshot.activeView) showView(snapshot.activeView === "sheets" ? "day" : snapshot.activeView);
  renderAll();
  hideUndoNotice();
}
...
function getAuthSession() {
  try {
    ...
    return {
      ...
      approvalStatus: user?.approvalStatus || session.approvalStatus || "approved",
      isApprovalAdmin: Boolean(user?.isApprovalAdmin || session.isApprovalAdmin || isApprovalAdminEmail(session.email)),
    };
  } catch {
    return null;
  }
...
async function ensureFreshAuthSession() {
  ...
  try {
    ...
    const nextSession = {
      ...
      name: user.name || session.name || "",
      approvalStatus: user.approvalStatus || session.approvalStatus || "approved",
      isApprovalAdmin: Boolean(user.isApprovalAdmin || session.isApprovalAdmin || isApprovalAdminEmail(user.email || session.email)),
      provider: "supabase",
...
function cacheAuthUser(session) {
  ...
  try {
    ...
    users[session.email] = {
      ...
      isApprovalAdmin: Boolean(session.isApprovalAdmin || isApprovalAdminEmail(session.email)),
      provider: "supabase",
...
function setTaskCompletionState(task = {}, completed = false) {
  ...
  TASK_COMPLETION_ALIAS_KEYS.forEach((key) => {
    task[key] = Boolean(completed);
  });
  return task;
}
...
function findCurrentTaskLocation(day, taskRef, fallbackPriority = "", fallbackIndex = -1, options = {}) {
  ...
  if (options.normalize !== false) normalizeDayTasks(day, options.dayKey || iso(selectedDate));
  if (taskRef.id) {
...
function normalizeDayTasks(day, dayKey = iso(selectedDate)) {
  if (!day) return;
  ...
  normalizeDeletedTaskIdentities(day);
  ...
  dedupeMaterializedCarryoverCopies(day, dayKey);
  alignDailyTaskPriorityLocations(day);
  compactEmptyDailyTaskSlots(day);
}

function alignDailyTaskPriorityLocations(day) {
  if (!day?.tasks) return false;
  const moves = [];
  priorities.forEach(([priority]) => {
    const list = day.tasks[priority] || [];
    list.forEach((task) => {
      if (!isActiveTaskSlot(task)) return;
      if (task.priorityUnset === true) return;
      const storedPriority = ["A", "B", "C"].includes(task.priority) ? task.priority : "";
      if (!storedPriority) {
        task.priority = priority;
        return;
      }
      if (storedPriority !== priority) moves.push({ fromPriority: priority, toPriority: storedPriority, task });
    });
  });
  if (!moves.length) return false;
  moves.forEach(({ fromPriority, toPriority, task }) => {
    const fromList = day.tasks[fromPriority] || [];
...
function normalizeDeletedFinanceTaskIds(day = {}) {
  ...
  day.deletedFinanceTaskIds = Array.from(new Set(day.deletedFinanceTaskIds.map(String).filter(Boolean)));
  return day.deletedFinanceTaskIds;
}
...
function normalizeDeletedTaskIdentities(day = {}) {
  if (!Array.isArray(day.deletedTaskIdentities)) {
    day.deletedTaskIdentities = [];
    return day.deletedTaskIdentities;
  }
  day.deletedTaskIdentities = Array.from(new Set(day.deletedTaskIdentities.map(String).filter(Boolean)));
  return day.deletedTaskIdentities;
}
...
function migrateState(nextState) {
  ...
  Object.entries(nextState.days || {}).forEach(([dayKey, day]) => {
    day.memo ||= "";
    day.memoTitle ||= "";
    ...
    normalizeDayTasks(day, dayKey);
    applyDeletedDailyTaskIdentities(day, dayKey);
    dedupeDailyTaskCopies(day, dayKey);
  });
  return nextState;
}

function preparePlannerStateForPersistence(nextState = state) {
  ...
  Object.entries(nextState.days || {}).forEach(([dayKey, day]) => {
    ...
    applyDeletedDailyTaskIdentities(day, dayKey);
    dedupeDailyTaskCopies(day, dayKey);
    purgePrematureCarryoverEntries(day, dayKey);
    clearOrphanTaskScheduleLinks(day);
    clearMisdatedDirectTaskScheduleLinks(day, dayKey);
  });
  return nextState;
}
  ...
  try {
    ...
    if (payload.exists && payload.state) {
      ...
      if (
        ...
        queueIncomingServerStateMerge(payload, readPlannerBaseState() || hydrationBaseState, "입력 완료 후 최신 데이터 병합")
      ) {
        return;
      }
...
async function persistStateToServer(options = {}) {
  ...
  if (meta.dirty && !hasPendingPlannerSave(meta)) {
    saveStateMeta({ dirty: false, accountEmail: getAuthSession()?.email || "" });
    saveStatus.saving = false;
    saveStatus.message = "최신 데이터 확인 중";
    renderSidebarAfterDailyInput();
...
async function pullServerStateIfNewer(options = {}) {
  ...
  const fetchBaseState = readPlannerBaseState() || clonePlannerValue(state);
  const fetchStartedMutationSeq = plannerMutationSeq;
  const session = await ensureFreshAuthSession();
  if (!session?.accessToken) {
    accountSaveReady = false;
    saveStatus.message = "다시 로그인 필요";
  ...
  const localMeta = getStateMeta();
  const hasPendingLocalSave = hasPendingPlannerSave(localMeta);
  if (localMeta.dirty && hasPendingLocalSave) {
    saveStatus.message = "저장 중";
    renderSidebarAfterDailyInput();
    scheduleAccountSave(options.force ? 120 : 250);
    return;
  }
  if (localMeta.dirty && !hasPendingLocalSave) {
    saveStateMeta({ dirty: false, accountEmail: getAuthSession()?.email || "" });
  }
  if (hasRecentDailyTaskMutation()) {
...
function stampTaskLifecycleMutation(task = {}) {
  if (!isPlainPlannerObject(task)) return task;
  const meta = getStateMeta();
  task.lifecycleUpdatedAt = nextPlannerMutationTimestamp(lastServerUpdatedAt, meta.updatedAt, task.lifecycleUpdatedAt);
  return task;
}
...
const TASK_LIFECYCLE_MERGE_KEYS = [
  "lifecycleUpdatedAt",
  "status",
  "priority",
  "priorityUnset",
...
function taskLifecycleChanged(baseTask = {}, candidateTask = {}) {
  return TASK_LIFECYCLE_MERGE_KEYS.some((key) => !plannerValuesEqual(baseTask?.[key], candidateTask?.[key]));
}

function copyTaskLifecycleFields(target = {}, source = {}) {
  TASK_LIFECYCLE_MERGE_KEYS.forEach((key) => {
    if (source[key] !== undefined) target[key] = clonePlannerValue(source[key]);
  });
  if (isTaskCompleted(source)) {
    setTaskCompletionState(target, true);
    target.status = "완료";
  } else if (["취소", "연기", "위임"].includes(source.status)) {
    setTaskCompletionState(target, false);
    target.status = source.status;
  } else if (source.status === "진행중") {
    setTaskCompletionState(target, false);
    target.status = "진행중";
  } else if (source.done === false) {
    setTaskCompletionState(target, false);
    target.status = source.status && source.status !== "완료" ? source.status : "미완료";
  }
  return target;
}
...
function mergePlannerTaskRecord(baseTask = {}, localTask = {}, remoteTask = {}) {
  ...
  const localLifecycleChanged = taskLifecycleChanged(baseTask || {}, localTask || {});
  const remoteLifecycleChanged = taskLifecycleChanged(baseTask || {}, remoteTask || {});
  let lifecycleWinner = winner;
  if (localLifecycleChanged && !remoteLifecycleChanged) lifecycleWinner = localTask;
  if (remoteLifecycleChanged && !localLifecycleChanged) lifecycleWinner = remoteTask;
  if (localLifecycleChanged && remoteLifecycleChanged) {
    const localLifecycleMs = timestampMs(localTask?.lifecycleUpdatedAt);
    const remoteLifecycleMs = timestampMs(remoteTask?.lifecycleUpdatedAt);
    if (localLifecycleMs || remoteLifecycleMs) {
      lifecycleWinner = localLifecycleMs >= remoteLifecycleMs ? localTask : remoteTask;
    }
  }
  if ((winnerRank > baseRank || localLifecycleChanged || remoteLifecycleChanged) && isPlainPlannerObject(lifecycleWinner)) {
    copyTaskLifecycleFields(merged, lifecycleWinner);
  }
...
function getPayloadPlannerState(payload) {
  ...
  try {
    return preparePlannerStateForPersistence(migrateState(clonePlannerValue(payload.state)));
  } catch {
    return null;
  }
}
...
function mergePlannerDeletionMarkers(targetState = {}, localState = {}, remoteState = {}) {
  ...
  const deletionKeys = ["deletedTaskIdentities", "deletedCarryoverTaskIdentities", "deletedFinanceTaskIds", "deletedRepeatIds"];
...
function plannerContentScore(source = state) {
  ...
  Object.values(source.days || {}).forEach((day) => {
    Object.values(day.tasks || {}).flat().forEach((task) => {
      score += hasText(task.text) || isTaskCompleted(task) || task.status !== "미완료" || hasText(task.delegate) ? 1 : 0;
    });
    score += countValues(Object.values(day.appointments || {}));
    score += Object.keys(day.appointmentMerges || {}).length;
    score += (day.deletedTaskIdentities || []).length;
...
function dayHasContent(day) {
  const hasText = (value) => String(value || "").trim().length > 0;
  const hasTask = Object.values(day.tasks || {}).flat().some((task) => hasText(task.text) || isTaskCompleted(task) || task.status !== "미완료" || hasText(task.delegate));
  ...
  const hasDeletionState = Boolean(
    (day.deletedTaskIdentities || []).length ||
...
function ensureWeek(key = weekKey()) {
  ...
  while (state.weeks[key].priorities.length < 5) state.weeks[key].priorities.push(setTaskCompletionState({ text: "", status: "미완료", priorityUnset: true }, false));
...
function ensureDay(key = iso(selectedDate)) {
  ...
  normalizeDeletedTaskIdentities(state.days[key]);
  ensureAppointmentSlots(state.days[key]);
  normalizeAppointmentMerges(state.days[key]);
  normalizeDayTasks(state.days[key], key);
  applyDeletedDailyTaskIdentities(state.days[key], key);
  dedupeDailyTaskCopies(state.days[key], key);
...
function createWeeklyPriorities(key, sourceState = null) {
  ...
  while (carried.length < 5) carried.push(setTaskCompletionState({ text: "", status: "미완료", priorityUnset: true }, false));
  return carried;
}
...
function removeCheckedWeeklyPriorityCarryovers(key, week, previousKey, previous = []) {
  ...
  week.priorities = week.priorities.map((item) => {
    ...
    if (text && checkedTexts.has(text) && !item.done && isCarried) {
      changed = true;
      return setTaskCompletionState({ text: "", status: "미완료", priorityUnset: true }, false);
    }
    return item;
  });
...
function normalizeWeeklyPriority(item = {}) {
  ...
  if (completed) {
    item.status = "완료";
    setTaskCompletionState(item, true);
  } else {
    setTaskCompletionState(item, false);
  }
  item.delegate ||= "";
  item.postponeDate ||= "";
...
function emptyTasks(count) {
  return Array.from({ length: count }, () =>
    setTaskCompletionState({ id: newTaskId(), text: "", status: "미완료", priorityUnset: true }, false),
  );
}
...
function setupSelectors() {
  ...
  if (el("approvalRefreshUsers")) el("approvalRefreshUsers").onclick = () => loadApprovalUsers({ force: true });
...
function handleOnboardingAction(action) {
  if (action === "task") {
    ...
    if (!hasEmptySlot) {
      const task = setTaskCompletionState({ text: "", status: "미완료", delegate: "", priorityUnset: true }, false);
      assignTaskOrder(day, task);
      day.tasks.A.push(task);
    }
    showView("day");
...
function renderSidebar() {
  ...
  if (saveStatusNode) {
    const fullStatusText = saveStatus.saving ? "저장 중" : saveStatus.message;
    const saveState = saveStatus.saving ? "saving" : /실패|오류|필요|로그인/.test(fullStatusText) ? "alert" : /대기|확인/.test(fullStatusText) ? "waiting" : "saved";
    saveStatusNode.dataset.saveState = saveState;
    saveStatusNode.textContent = compactSaveStatusLabel(saveState);
...
function compactSaveStatusLabel(stateName = "waiting") {
  if (stateName === "saving") return "저장중";
  if (stateName === "alert") return "주의";
  if (stateName === "saved") return "저장됨";
  return "확인";
}
...
function isApprovalAdminEmail(email = "") {
  return String(email || "").trim().toLowerCase() === "j3010@ymail.com";
}

function isApprovalAdminSession() {
  const session = getAuthSession();
  return Boolean(session?.isApprovalAdmin || isApprovalAdminEmail(session?.email));
}
...
function renderApprovalAdminSettings(message = "") {
  const option = el("approvalAdminOption");
  if (!option) return;
  const isAdmin = isApprovalAdminSession();
  option.hidden = !isAdmin;
  if (!isAdmin) return;
  const status = el("approvalAdminStatus");
  if (message && status) status.textContent = message;
  renderApprovalUserList();
  if (!approvalUsersLoaded && !approvalUsersLoading) loadApprovalUsers();
}

async function loadApprovalUsers(options = {}) {
  if (!isApprovalAdminSession()) return;
  if (approvalUsersLoading && !options.force) return;
  approvalUsersLoading = true;
  renderApprovalAdminSettings("승인 목록을 불러오는 중입니다.");
  try {
    const session = await ensureFreshAuthSession();
    if (!session?.accessToken) throw new Error("로그인 세션이 필요합니다.");
    const response = await fetch("/api/admin-users", { cache: "no-store", headers: authStateHeaders() });
    const payload = await response.json().catch(() => ({}));
...
function renderApprovalUserList() {
  ...
  list.innerHTML = approvalUsers.map((user) => {
    const status = normalizeApprovalStatus(user.approval_status || user.approvalStatus);
    const tier = normalizeAccountTier(user.tier);
    return `
      <article class="approval-user-card" data-user-id="${escapeAttr(user.user_id || user.userId || "")}">
        ...
        <label>
          <span>등급</span>
          <select data-approval-tier>
            ${["ceo", "director", "manager", "staff"].map((value) => `<option value="${value}" ${tier === value ? "selected" : ""}>${formatTierName(value)}</option>`).join("")}
          </select>
        </label>
        <label>
          <span>상태</span>
          <select data-approval-status>
            ${["pending", "approved", "suspended", "rejected"].map((value) => `<option value="${value}" ${status === value ? "selected" : ""}>${formatApprovalStatus(value)}</option>`).join("")}
          </select>
        </label>
        <button class="mini-action" type="button" data-approval-save>적용</button>
      </article>
    `;
  }).join("");
...
async function saveApprovalUser(card) {
  ...
  const tier = card.querySelector("[data-approval-tier]")?.value || "staff";
  const approvalStatus = card.querySelector("[data-approval-status]")?.value || "pending";
  if (!userId) return renderApprovalAdminSettings("사용자 ID를 확인할 수 없습니다.");
  renderApprovalAdminSettings("승인 상태를 저장하는 중입니다.");
  try {
    const session = await ensureFreshAuthSession();
    if (!session?.accessToken) throw new Error("로그인 세션이 필요합니다.");
    const response = await fetch("/api/admin-users", {
      method: "POST",
      headers: authStateHeaders({ "Content-Type": "application/json" }),
      body: JSON.stringify({ userId, tier, approvalStatus }),
    });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(payload.error || payload.detail || "승인 상태를 저장하지 못했습니다.");
    approvalUsersLoaded = false;
    await loadApprovalUsers({ force: true });
  } catch (error) {
    renderApprovalAdminSettings(error.message || "승인 상태를 저장하지 못했습니다.");
  }
}
...
function formatApprovalStatus(status = "pending") {
  return {
    pending: "승인 대기",
    approved: "승인됨",
    rejected: "거절",
    suspended: "정지",
  }[normalizeApprovalStatus(status)] || "승인 대기";
}

function formatDateTimeShort(value = "") {
  if (!value) return "가입일 없음";
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "가입일 없음";
...
function applyLanguagePreference() {
  ...
  normalizePrimaryNavigationLabels(language);
}

function normalizePrimaryNavigationLabels(language = getAppLanguage()) {
  const labels = languageLabels[language] || languageLabels.en;
  const topLabels = {
    day: labels.day,
    week: labels.week,
  ...
  Object.entries(topLabels).forEach(([view, label]) => {
    document.querySelectorAll(`button[data-top-view="${view}"], .tab[data-view="${view}"]`).forEach((item) => {
      if (!label) return;
...
function getUpcomingMoneyRisks(anchorDate = selectedDate, days = 7) {
  ...
  Object.entries(state.finance.months || {}).forEach(([key, rows]) => {
    rows.forEach((item) => {
      ...
      if (!item.title?.trim() || !date || date < start || date > end || isTaskCompleted(item)) return;
  ...
  (state.finance.fixed || []).forEach((item) => {
    ...
    fixedMoneyTaskMonthKeys().forEach((key) => {
      const date = getMoneyItemDate(item, key);
      if (!date || date < start || date > end || !isFixedMoneyActiveForMonth(item, key) || isTaskCompleted(item)) return;
...
function addSuggestedTask(text) {
  ...
  const task = setTaskCompletionState({ text, status: "미완료", priorityUnset: false }, false);
  assignTaskOrder(day, task);
...
function addTaskToDate(dateKey, text, priority = suggestedTaskPriority(text)) {
  ...
  const task = setTaskCompletionState({ id: newTaskId(), text: text.trim(), status: "미완료", priorityUnset: false }, false);
  normalizeTask(task);
  assignTaskOrder(day, task);
...
function renderDayCompass() {
  ...
  week.priorities.forEach((item, index) => {
    item = normalizeWeeklyPriority(item || setTaskCompletionState({ text: "", status: "미완료", priorityUnset: true }, false));
  ...
  addPriority.onclick = () => {
    week.priorities.push(setTaskCompletionState({ id: newTaskId(), text: "", status: "미완료", priorityUnset: true }, false));
    saveState({ fastSave: true });
    renderDayCompass();
    window.requestAnimationFrame(() => {
...
function scheduleWeeklyPriorityPostpone(item, targetDate) {
  ...
  if (!existingTask && text) {
    existingTask = setTaskCompletionState({
      id: newTaskId(),
      text,
      status: "미완료",
      priorityUnset: false,
    ...
    setTaskCompletionState(existingTask, false);
...
function getTaskMarker(task) {
  ...
  if (isTaskCompleted(task)) return "check";
  return "empty";
}
...
function runTaskCycleOnce(anchor, handler) {
  ...
  if (anchor.dataset.cycleLock === "1") return;
  anchor.dataset.cycleLock = "1";
  anchor.setAttribute("aria-busy", "true");
  try {
    handler();
  } finally {
    window.setTimeout(() => {
      delete anchor.dataset.cycleLock;
...
function getPriorityMenuValue(task, priority) {
  ...
  const storedPriority = ["A", "B", "C"].includes(task.priority) ? task.priority : "";
  const locationPriority = ["A", "B", "C"].includes(priority) ? priority : "";
  return storedPriority || locationPriority || "선택";
}

function shouldStrikeTask(task) {
  return isTaskCompleted(task) || ["위임", "취소", "연기"].includes(task.status);
}
...
function isTaskCompleted(task = {}) {
  ...
  if (
    ...
    status === "완료됨" ||
    ["done", "complete", "completed", "checked"].includes(normalized)
  ) {
    return true;
  }
...
function getStoredTaskPriority(task = {}, fallbackPriority = "") {
  return [task.priority, task.postponePriority, task.originalPriority, fallbackPriority]
    .find((value) => ["A", "B", "C"].includes(value)) || "";
}

function getPostponeRestorePriority(task = {}, fallbackPriority = "") {
  return [task.postponePriority, task.priority, task.originalPriority, fallbackPriority]
    .find((value) => ["A", "B", "C"].includes(value)) || "";
}
...
function handlePriorityMenuChange(task, fromPriority, index, value) {
  ...
  if (["위임", "취소", "연기"].includes(value)) {
    if (value === "연기") {
      targetTask.postponePriority = getStoredTaskPriority(targetTask, location?.priority || fromPriority || "");
  ...
  setTaskCompletionState(targetTask, isTaskCompleted(targetTask));
  if (["A", "B", "C"].includes(value)) {
...
function deleteTask(priority, index, taskRef = null) {
  ...
  if (task.financeItemId) {
    markFinanceTaskDeletedForDate(task.financeItemId, iso(selectedDate));
  }
...
function schedulePostponedTask(task, priority, targetDate) {
  ...
  const storedPriority = getPostponeRestorePriority(task, priority);
  const targetPriority = storedPriority || "A";
  const targetPriorityUnset = Boolean(task.postponePriorityUnset && !storedPriority);
  clearTaskScheduleLinkForInactive(task, sourceDay);
  task.status = "연기";
  setTaskCompletionState(task, false);
  task.postponeDate = targetDate;
  task.postponeMode = "";
  task.postponeId ||= `postpone-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  task.postponePriority = storedPriority;
  task.postponePriorityUnset = targetPriorityUnset;
  task.carryoverDeletedFrom = sourceKey;
  stampTaskLifecycleMutation(task);
  const sourceTaskRef = { ...task, priority: storedPriority || priority, date: sourceKey };
  markCarryoverIdentityDeletedForDate(sourceTaskRef, getCarryoverDeleteFromKey(sourceKey));
  removeCarryoverCopiesMatchingRef(sourceTaskRef, getCarryoverDeleteFromKey(sourceKey), {
    preserveTask: task,
    preserveTaskDayKey: sourceKey,
  });
  const targetDay = ensureDay(targetDate);
  ...
  if (!existingTask && task.text?.trim()) {
    ...
    existingTask.originalPriority = targetPriority;
    stampTaskLifecycleMutation(existingTask);
    if (existingPriority && existingPriority !== targetPriority) {
...
function removePostponedTaskOccurrence(postponeId, keepDate = "") {
  ...
  Object.entries(state.days || {}).forEach(([key, day]) => {
    ...
    normalizeDayTasks(day, key);
    priorities.forEach(([priority]) => {
      day.tasks[priority] = day.tasks[priority].filter((item) => {
        if (item.postponedFrom !== postponeId) return true;
        markDailyTaskDeletedForDate(day, item, priority, key);
        return false;
      });
    });
  });
...
function findTaskSource(taskRef) {
  ...
  if (!day) return null;
  normalizeDayTasks(day, taskRef.date);
  if (taskRef.id) {
    for (const [priority] of priorities) {
...
function dedupeMaterializedCarryoverCopies(day, dayKey = iso(selectedDate)) {
  if (!day?.tasks) return false;
  const byIdentity = new Map();
  ...
  priorities.forEach(([priority]) => {
    ...
    day.tasks[priority] = list.filter((task) => {
      ...
      normalizeTask(task);
      const taskWithContext = { ...task, priority, date: dayKey };
      const identities = Array.from(new Set([
        getCarryoverTaskIdentity(taskWithContext),
        getCarryoverSemanticIdentity(taskWithContext),
        ...getCarryoverIdentityValues(taskWithContext),
        ...getDailyTaskDuplicateIdentities(task, priority, dayKey),
      ].filter(Boolean)));
...
function getDailyTaskDuplicateIdentities(task = {}, priority = "", dayKey = iso(selectedDate)) {
  ...
  const identities = [];
  if (task.id) identities.push(`id:${task.id}`);
...
function getDailyTaskDeletionIdentities(task = {}, priority = "", dayKey = iso(selectedDate)) {
  if (!isPlainPlannerObject(task)) return [];
  const identities = [];
  [
    ["id", task.id],
    ["task", task.taskId],
    ["repeat", task.repeatId],
    ["finance", task.financeItemId],
    ["project", task.projectTaskId],
    ["postpone", task.postponedFrom || task.postponeId],
    ["carryover", task.carryoverForkFrom],
  ].forEach(([prefix, value]) => {
    if (value !== undefined && value !== null && String(value).trim()) identities.push(`${prefix}:${String(value)}`);
  });
  if (isMaterializedCarryoverTask(task)) {
    identities.push(...getCarryoverIdentityValues({ ...task, priority, date: dayKey }));
  }
  const text = normalizeSearchText(task.text || "");
  if (text && (isMaterializedCarryoverTask(task) || isSystemManagedDailyTaskCandidate(task))) {
    identities.push(`text:${text}`, `task-text:${text}`);
    const storedPriority = getStoredTaskPriority(task, priority || task.priority || "");
    if (storedPriority) identities.push(`text:${text}:${storedPriority}`);
  }
  return Array.from(new Set(identities.filter(Boolean)));
}

function markDailyTaskDeletedForDate(day = ensureDay(), task = {}, priority = "", dayKey = iso(selectedDate)) {
  if (!day || !isPlainPlannerObject(task)) return false;
  const list = normalizeDeletedTaskIdentities(day);
  const identities = getDailyTaskDeletionIdentities(task, priority || task.priority || "", dayKey);
  let changed = false;
...
function applyDeletedDailyTaskIdentities(day = {}, dayKey = iso(selectedDate)) {
  if (!day?.tasks) return false;
  const deleted = new Set(normalizeDeletedTaskIdentities(day));
  ...
  priorities.forEach(([priority]) => {
    ...
    const next = list.filter((task) => {
      ...
      const identities = getDailyTaskDeletionIdentities(task, priority, dayKey);
      const shouldDelete = identities.some((identity) => deleted.has(identity));
      if (shouldDelete) {
        clearTaskScheduleLinkForInactive(task, day);
        changed = true;
        return false;
...
function mergeDuplicateDailyTaskState(target = {}, duplicate = {}) {
  ...
  const duplicateLifecycleMs = timestampMs(duplicate.lifecycleUpdatedAt);
  const targetLifecycleMs = timestampMs(target.lifecycleUpdatedAt);
  const duplicateHasNewerLifecycle =
    duplicateLifecycleMs &&
    (!targetLifecycleMs || duplicateLifecycleMs >= targetLifecycleMs) &&
    taskLifecycleChanged(target, duplicate);
  if (duplicateHasNewerLifecycle || getTaskResolutionRank(duplicate) > getTaskResolutionRank(target)) {
    copyTaskLifecycleFields(target, duplicate);
  }
    ...
    priorities.forEach(([priority]) => {
      ...
      const next = list.filter((candidate) => {
        ...
        const matches = Boolean(
          candidateIdentities.some((identity) => removalIdentities.has(identity)) ||
          (targetText && candidateText === targetText && candidateIsRemovable)
        );
        if (!matches) return true;
        clearTaskScheduleLinkForInactive(candidate, day);
        clearTaskTextTimeHintFromSchedule(candidate.text, day, { linkId: getTaskScheduleLinkId(candidate) });
        return false;
      });
      if (next.length !== list.length) {
        day.tasks[priority] = next;
...
function markCarryoverDeletedFromDate(taskRef = {}, deleteFromKey = iso(selectedDate), fallbackSource = null, options = {}) {
  ...
  stampTaskLifecycleMutation(source.task);
...
function deleteMaterializedCarryoverTask(location) {
  ...
  markDailyTaskDeletedForDate(day, task, location?.priority || task.priority || "", selectedKey);
  applyDeletedDailyTaskIdentities(day, selectedKey);
...
function materializeCarryoverTask(taskRef, targetKey = iso(selectedDate)) {
  ...
  let targetPriority = getStoredTaskPriority(source.task, taskRef.priority || source.priority || "") || "A";
  let targetTask = null;
  ...
  const sourcePriorityUnset = Boolean(
    source.task.priorityUnset === true ||
      (taskRef.priorityUnset === true && !getStoredTaskPriority(source.task, taskRef.priority || source.priority || "")),
  );
  if (!targetTask) {
    targetTask = {
      ...source.task,
    ...
    targetTask.priority = sourcePriorityUnset ? "" : targetPriority;
    targetTask.priorityUnset = sourcePriorityUnset;
    setTaskCompletionState(targetTask, false);
  ...
  } else if (!targetTask.priorityUnset) {
    targetTask.priority = ["A", "B", "C"].includes(targetTask.priority) ? targetTask.priority : targetPriority;
  }
...
function deleteCarryoverTask(taskRef) {
  ...
  markDailyTaskDeletedForDate(selectedDay, taskRef, taskRef.priority || source?.priority || "", selectedKey);
  markCarryoverDeletedFromDate(taskRef, deleteFromKey, source);
  removeCarryoverCopiesMatchingRef(taskRef, deleteFromKey);
  applyDeletedDailyTaskIdentities(selectedDay, selectedKey);
...
function updateCarryoverTaskPriority(taskRef, value, targetKey = iso(selectedDate)) {
  ...
  if (isInactiveValue) {
    if (value === "연기") {
      source.task.postponePriority = getStoredTaskPriority(source.task, source.priority || taskRef.priority || "");
  ...
  setTaskCompletionState(source.task, isTaskCompleted(source.task));
  if (["A", "B", "C"].includes(value)) {
    source.task.priorityUnset = false;
...
function renderProjectBoard() {
  ...
  state.projects.items.forEach((project, index) => {
    ...
    item.querySelector(".project-list-open").onclick = () => {
...
function addProjectNextActionToToday(project) {
  if (!project) return;
  const title = projectTitle(project);
  const action = project.nextAction?.trim() || "다음 행동 정하기";
  const text = `프로젝트: ${title} - ${action}`;
  captureUndo("프로젝트 오늘 업무 추가");
  const day = ensureDay(iso(selectedDate));
  const existingRef = priorities
    .map(([priority]) => ({ priority, task: (day.tasks?.[priority] || []).find((item) => item.projectQuickTaskId === project.id) }))
    .find((item) => item.task);
  const task = existingRef?.task || setTaskCompletionState({ text: "", status: "미완료", priorityUnset: false }, false);
  task.text = text;
  task.status = "미완료";
  setTaskCompletionState(task, false);
  task.priorityUnset = false;
  task.projectQuickTaskId = project.id;
  task.projectSourceTitle = title;
  if (!Number.isFinite(Number(task.order))) assignTaskOrder(day, task);
  if (!existingRef) day.tasks.B.push(task);
  saveState();
  showView("day");
  renderAll();
  showUndoNotice("프로젝트 다음 행동을 오늘 우선업무에 넣었습니다.");
}
...
function linkProjectToTask(project) {
  ...
  const task =
    targetDay.tasks.B.find((item) => item.projectTaskId === project.id) ||
    targetDay.tasks.B.find((item) => !item.text?.trim()) ||
    setTaskCompletionState({ text: "", status: "미완료", priorityUnset: false }, false);
...
function isFinanceTaskDeletedForDate(linkId = "", dayKey = "") {
  if (!linkId || !dayKey) return false;
  const deleted = state.days?.[dayKey]?.deletedFinanceTaskIds;
  return Array.isArray(deleted) && deleted.some((id) => String(id) === String(linkId));
}

function markFinanceTaskDeletedForDate(linkId = "", dayKey = iso(selectedDate)) {
  if (!linkId || !dayKey) return false;
  const day = ensureDay(dayKey);
  normalizeDeletedFinanceTaskIds(day);
  if (day.deletedFinanceTaskIds.includes(String(linkId))) return false;
  day.deletedFinanceTaskIds.push(String(linkId));
  return true;
}
...
function linkMoneyItemToTask(item, key, linkId = item.id, fixed = false) {
  ...
  if (isFinanceTaskDeletedForDate(linkId, targetDate)) {
  ...
  const task = targetTask || setTaskCompletionState({ text: "", status: "미완료", priorityUnset: false }, false);
  task.text = taskText;
  task.status = isTaskCompleted(item) ? "완료" : "미완료";
  setTaskCompletionState(task, isTaskCompleted(item));
  task.priorityUnset = false;
...
function getDayTasks(key) {
  ...
  if (!day) return [];
  normalizeDayTasks(day, key);
  applyDeletedDailyTaskIdentities(day, key);
  dedupeDailyTaskCopies(day, key);
  return priorities
...
function buildLocalQuestionAnswer(query) {
  const question = normalizeSearchText(query);
  if (!question) return "";
  const key = iso(selectedDate);
  const weekStart = startOfWeek(selectedDate);
  const weekEnd = new Date(weekStart);
  ...
  const wantsWeek = /이번\s*주|주간|week/.test(question);
  const wantsToday = /오늘|금일|today/.test(question);
  const wantsMoney = /money|자금|돈|카드|지출|수입|이자|대금|입금|납부|상환/.test(question);
  const wantsProject = /프로젝트|project/.test(question);
  const wantsSchedule = /일정|시간|언제|다음|schedule|when/.test(question);
  const wantsTask = /업무|할\s*일|해야|중요|우선|task|todo|뭐|무엇/.test(question);
  const terms = getSearchTerms(query);
  const lines = [];
  ...
  if (wantsToday || wantsTask || wantsSchedule) {
    const day = ensureDay(key);
    const tasks = getDayTasks(key).filter((task) => task.text?.trim() && !shouldStrikeTask(task));
    const appointments = Object.entries(day.appointments || {})
      .filter(([, text]) => String(text || "").trim())
      .sort(([a], [b]) => timeToMinutes(a) - timeToMinutes(b));
    tasks.slice(0, 5).forEach((task) => addLine(`${formatShortDate(selectedDate)} ${task.priority || "?"} · ${stripTaskTimeText(task.text)}`));
    appointments.slice(0, 5).forEach(([time, text]) => addLine(`${formatShortDate(selectedDate)} ${time} · ${stripTaskTimeText(text)}`));
  }

  if (wantsWeek) {
    const week = ensureWeek(weekKey(selectedDate));
    (week.priorities || [])
      .filter((item) => item?.text && !item.done)
      .slice(0, 5)
      .forEach((item) => addLine(`${formatShortDate(weekStart)}~${formatShortDate(weekEnd)} · ${item.text}`));
    for (let i = 0; i < 7 && lines.length < 8; i += 1) {
      const date = new Date(weekStart);
      date.setDate(weekStart.getDate() + i);
      getDayTasks(iso(date))
        .filter((task) => task.text?.trim() && !shouldStrikeTask(task))
        .slice(0, 2)
        .forEach((task) => addLine(`${formatShortDate(date)} ${task.priority || "?"} · ${stripTaskTimeText(task.text)}`));
    }
  }

  if (wantsMoney) {
    const moneyRows = [];
    Object.entries(state.finance?.months || {}).forEach(([month, rows]) => {
      rows.forEach((item) => {
        if (item?.title) moneyRows.push({ date: `${month}-${pad(Number(item.dueDay) || 1)}`, item, fixed: false });
      });
    });
    (state.finance?.fixed || []).forEach((item) => {
      if (item?.title) moneyRows.push({ date: item.taskDate || key, item, fixed: true });
    });
    moneyRows
      .filter(({ item, date }) => matchesSearchText(`${item.title} ${item.category || ""} ${item.amount || ""} ${date}`, terms) || wantsMoney)
      .sort((a, b) => String(a.date).localeCompare(String(b.date)))
      .slice(0, 6)
      .forEach(({ date, item, fixed }) => {
        const amountText = item.amount ? ` ${formatMoneyAmount(parseMoneyAmount(item.amount))}` : "";
        const repeatText = fixed ? " 반복" : "";
        addLine(`${date} Money · ${item.title}${amountText}${repeatText}`);
      });
  }
  ...
  if (wantsProject) {
    (state.projects?.items || [])
      ...
      .slice(0, 6)
      .forEach((project) => {
        const actionText = project.nextAction ? ` / 다음 행동: ${project.nextAction}` : "";
        addLine(`${project.status} · ${projectTitle(project)}${actionText}`);
      });
  }

  if (!lines.length) {
    collectSearchResults(query).slice(0, 5).forEach((result) => {
      addLine(`${result.date || result.type} · ${result.label}: ${String(result.text || "").slice(0, 52)}`);
    });
  }

  if (!lines.length) return "";
  const heading = isKo ? "앱 기록 기준 요약" : "Planner summary";
...
async function requestAiSearchAnswer(value) {
  ...
  if (!question) return;
  const localAnswer = buildLocalQuestionAnswer(question);
  aiSearch = { query: question, answer: localAnswer, loading: true, error: "" };
  showView("search");
  renderSearch();
  try {
    const session = getAuthSession();
    ...
    aiSearch = {
      ...
      answer: localAnswer,
      loading: false,
      error: localAnswer ? "" : `${error.message || "AI 연결 오류"} · 배포 환경에서는 OPENAI_API_KEY와 /api/ask 설정을 확인하세요.`,
    };
  }
  renderSearch();
}
...
function renderSearch() {
  ...
  const results = searchQuery ? collectSearchResults(searchQuery) : [];
  if (aiSearch.query) {
    const answerText = aiSearch.loading
      ? (aiSearch.answer || "AI가 플래너 내용을 읽고 답변을 준비하고 있습니다.")
      : (aiSearch.error || aiSearch.answer);
    const card = document.createElement("article");
    card.className = `search-result ai-answer ${aiSearch.loading ? "is-loading" : ""}`;
    card.innerHTML = `
      <strong>AI 답변</strong>
      <small>${escapeAttr(aiSearch.query)}</small>
      <p>${escapeAttr(answerText)}</p>
    `;
    ...
    const localAnswer = buildLocalQuestionAnswer(searchQuery);
    if (localAnswer) {
      const card = document.createElement("article");
      card.className = "search-result ai-answer local-answer";
      card.innerHTML = `
        <strong>질문 해석</strong>
        <small>${escapeAttr(searchQuery)}</small>
        <p>${escapeAttr(localAnswer)}</p>
      `;
      node.appendChild(card);
    }
  }
...
function buildPlannerWorkbookTables() {
  ...
  Object.keys(state.days || {}).sort().forEach((key) => {
    getDayTasks(key).filter((task) => task.text || isTaskCompleted(task) || task.status !== "미완료").forEach((task) => {
      taskRows.push([key, task.priority, task.status || "", isTaskCompleted(task) ? "완료" : "", task.text || "", task.delegate || ""]);
    });
  });
