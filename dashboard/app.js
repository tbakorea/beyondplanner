...
function setupSelectors() {
  ...
  el("excelExportButton").onclick = exportPlannerExcel;
...
function exportPlannerExcel() {
  const sheets = buildPlannerExcelSheets();
  const workbook = buildExcelXmlWorkbook(sheets);
  const blob = new Blob([workbook], { type: "application/vnd.ms-excel;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `beyond-work-backup-${iso(todayInPlanner())}.xls`;
  link.click();
  URL.revokeObjectURL(url);
}

function buildPlannerExcelSheets() {
  const dailyTasks = [["날짜", "우선순위", "상태", "업무내용", "완료", "이월/반복", "연결태그"]];
  Object.entries(state.days || {}).sort(([a], [b]) => a.localeCompare(b)).forEach(([key]) => {
    getDayTasks(key).forEach((task) => {
      ...
      dailyTasks.push([
        ...
        task.text || "",
        task.done ? "완료" : "",
        task.repeatId ? "반복" : task.carryoverSourceDate ? "이월" : "",
        getTaskLinkTags(task).join(", "),
      ]);
    });
  });

  const schedule = [["날짜", "시간", "일정내용", "병합칸"]];
  Object.entries(state.days || {}).sort(([a], [b]) => a.localeCompare(b)).forEach(([key, day]) => {
    const slots = getScheduleSlotsForDay(day);
    slots.forEach((slot) => {
      const text = String(day.appointments?.[slot] || "").trim();
      if (!text || isCoveredAppointmentSlot(day, slot, slots)) return;
      schedule.push([key, slot, text, String(getAppointmentSpan(day, slot) || 1)]);
    });
  });

  const weekly = [["주 시작일", "구분", "역할/항목", "목표/내용", "핵심행동", "완료"]];
  Object.entries(state.weeks || {}).sort(([a], [b]) => a.localeCompare(b)).forEach(([key, week]) => {
    (week.priorities || []).forEach((item, index) => {
      if (!item.text?.trim()) return;
      weekly.push([key, "금주의 주요일정", `주요일정 ${index + 1}`, item.text, "", item.done ? "완료" : ""]);
    });
    (week.compass || []).forEach((item) => {
      normalizeCompassItem(item);
      if (!item.goal && !(item.actions || []).some(Boolean)) return;
      weekly.push([key, "위클리 콤파스", item.role || "", item.goal || "", (item.actions || []).filter(Boolean).join(" / "), ""]);
    });
  });

  const monthly = [["월", "월간초점", "프로젝트/계획", "기념일"]];
  Object.entries(state.months || {}).sort(([a], [b]) => a.localeCompare(b)).forEach(([key, month]) => {
    const anniversaries = (state.calendar?.events || [])
      .filter((event) => String(event.date || "").startsWith(key))
      .map((event) => `${event.date} ${event.title}`)
      .join(" / ");
    monthly.push([key, month.focus || "", (month.projects || []).filter(Boolean).join(" / "), anniversaries]);
  });

  const money = [["구분", "월", "수입/지출", "내용", "금액", "일자", "상태", "종료", "메모"]];
  Object.entries(state.finance?.months || {}).sort(([a], [b]) => a.localeCompare(b)).forEach(([key, rows]) => {
    rows.forEach((item) => {
      if (!hasMoneyItemContent(item)) return;
      money.push(["월별", key, item.type || "", item.title || "", item.amount || "", item.dueDay || item.taskDate || "", item.status || "", "", item.memo || ""]);
    });
  });
  (state.finance?.fixed || []).forEach((item) => {
    if (!hasMoneyItemContent(item)) return;
    money.push(["반복", "", item.type || "", item.title || "", item.amount || "", item.dueDay || "", item.status || "", item.repeatEndMode === "date" ? item.repeatEndDate || "" : "종료없음", item.memo || ""]);
  });

  const projects = [["프로젝트명", "상태", "담당", "목표", "다음행동", "시작일", "종료/마감", "메모", "자금 시뮬레이션"]];
  (state.projects?.items || []).forEach((project) => {
    const projectName = project.title || project.name || "";
    if (!projectName && !project.goal && !project.nextAction) return;
    const finances = (project.finances || [])
      .filter((item) => item.title || item.amount)
      .map((item) => `${item.type || ""} ${item.title || ""} ${item.amount || ""} ${item.probability || ""} ${item.timing || ""}`.trim())
      .join(" / ");
    projects.push([projectName, project.status || "", project.owner || "", project.goal || "", project.nextAction || "", project.startDate || "", project.endDate || project.dueDate || "", project.notes || "", finances]);
  });

  const review = [["날짜", "메모", "일일기록", "완료/성과", "내일로 넘길 일", "개선할 점"]];
  Object.entries(state.days || {}).sort(([a], [b]) => a.localeCompare(b)).forEach(([key, day]) => {
    if (![day.memo, day.record, day.wins, day.carry, day.lesson].some((value) => String(value || "").trim())) return;
    review.push([key, day.memo || "", day.record || "", day.wins || "", day.carry || "", day.lesson || ""]);
  });

  const profile = [
    ["항목", "내용"],
    ["주 정체성", getPersonaLabel(state.profile)],
    ["보조 정체성", getSecondaryPersonaLabels(state.profile).join(", ")],
    ["유형 설명", state.profile?.personaCustom || ""],
    ["나이", state.profile?.age || ""],
    ["직업", state.profile?.job || ""],
    ["현재 역할", state.profile?.roles || ""],
    ["핵심 목표", state.profile?.goals || ""],
    ["의사결정 기준", state.profile?.decisionPrinciples || ""],
    ["현재 과제", state.profile?.currentChallenges || ""],
    ["건강 상태", state.profile?.healthStatus || ""],
    ["운동 목표", state.profile?.exerciseGoal || ""],
  ];

  return [
    { name: "Daily Tasks", rows: dailyTasks },
    { name: "Schedule", rows: schedule },
    { name: "Weekly", rows: weekly },
    { name: "Monthly", rows: monthly },
    { name: "Money", rows: money },
    { name: "Projects", rows: projects },
    { name: "Review", rows: review },
    { name: "Profile", rows: profile },
  ];
}
